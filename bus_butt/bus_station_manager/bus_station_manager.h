 ...
 * @brief 정류장의 전체 논리적 상태를 관리하고, 센서 데이터 변화를 분석하여
...
private:
    ...
    // --- 안정화를 위한 상태 ---
    std::vector<int> last_seen_sensor_status_;  // 마지막으로 확인한 물리적 센서 상태
    std::chrono::time_point<std::chrono::steady_clock> last_sensor_change_time_; // 센서 상태가 마지막으로 바뀐 시간
    std::vector<int> last_confirmed_status_;    // 마지막으로 '확정' 처리한 상태 (중복 처리 방지용)

    const int REENTRY_COOLDOWN_S = 5;           // 재정차로 간주할 최대 시간 (초)
    const int CONFIRMATION_TIME_S = 2;          // 상태 안정화에 필요한 시간 (초)
...
public:
    ...
    void updateSensorState(const std::vector<int>& current_status);
