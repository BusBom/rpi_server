...
// 캡처 전용 스레드 함수
void capture_thread() {
    // Reverting to V4L2 backend directly for potentially better performance and compatibility
    // Explicitly setting CAP_PROP_FOURCC to MJPG for hardware acceleration if available
    // Setting FPS for desired frame rate
    cv::VideoCapture cap(0, cv::CAP_V4L2); // Use 0 for default camera, or "/dev/video0" if 0 doesn't work
    
    if (!cap.isOpened()) {
        std::cerr << "카메라 연결 실패: V4L2 백엔드로 카메라를 열 수 없습니다. 카메라 장치 경로, 권한 또는 모듈 활성화 상태를 확인하세요." << std::endl; 
        return;
    }

    std::cout << "카메라 연결 성공, 프레임 수신 중 ..." << std::endl;
    cap.set(cv::CAP_PROP_BUFFERSIZE, 1);              // 내부 버퍼 최소화
    cap.set(cv::CAP_PROP_FRAME_WIDTH, FRAME_WIDTH);   // 프레임 너비 설정
    cap.set(cv::CAP_PROP_FRAME_HEIGHT, FRAME_HEIGHT); // 프레임 높이
    cap.set(cv::CAP_PROP_FOURCC, cv::VideoWriter::fourcc('M', 'J', 'P', 'G')); // Request MJPEG format
    cap.set(cv::CAP_PROP_FPS, 10); // Set desired frame rate

    cv::Mat frame;                                    // 프레임 저장용

    while (running.load()) {
        cap >> frame;                                // PICAM에서 한 프레임 읽기
...
void writer_thread(cv::VideoWriter* writer) {
    while (running.load()) {
        ...
        if (!frame_queue.empty()) {
            ...
            size_t data_size = latest.total() * latest.elemSize();  // 데이터 크기 계산
            std::memcpy(shm_ptr, latest.data, data_size);           // SHM에 바이트 복사
